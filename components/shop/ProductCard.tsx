...
export default function ProductCard({ id, name, price, image_url, availability, stock, sunlight, water, careNotes }: ProductCardProps) {
  ...
  const displayAvailability = availability === 'Coming Soon' ? 'Coming Soon' : stock === 0 ? 'Out of Stock' : availability;

  return (
    <>
      <div className="bg-[var(--card-bg)] rounded-lg border border-[var(--card-border)] shadow-sm hover:shadow-md transition-shadow overflow-hidden group cursor-pointer" onClick={() => setIsModalOpen(true)}>
        {/* image */}
